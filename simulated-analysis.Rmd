...
student.model.moderator.estimates <- student.moderator.model.results %>%
  ...
  group_by(class.id, incentive) %>%
  sample_n(20) %>%
  mutate(slope = moderator.effect / sd(student.class.data[[student.moderator.name]])) %>%
  mutate(intercept = mu.class - moderator.effect * mean(student.class.data[[student.moderator.name]])/sd(student.class.data[[student.moderator.name]])) %>%
  ungroup() %>%
  mutate(incentive.mcmc = factor(incentive, labels=c("Non-incentivized", "Incentivized"))) %>%
  select(-incentive) %>%
  left_join(select(classes, class.id, incentive), by="class.id") %>%
  filter(incentive == incentive.mcmc) %>%
  select(-incentive.mcmc)
...
student.model.moderator.estimates <- student.moderator.model.results %>%
  spread_draws(mu.class[class.id], moderator.effect[incentive, moderator.level]) %>%
  ungroup() %>%
  mutate(incentive = factor(incentive, labels=c("Non-incentivized", "Incentivized"))) %>%
...
ggplot(student.class.data, aes(x=access.all.delayed.feedback, y=delta.z, color=incentive))+
  ...
  geom_crossbar(data=student.model.moderator.estimates, aes(x=access.all.delayed.feedback, y = mu.class + moderator.effect, color=incentive, ymin=..y.., ymax=..y..), position=position_dodge(width=0.75), width=0.5, size=0.1) +
