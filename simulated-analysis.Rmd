...
```{r}
estimate.student.moderator.effect <- function(moderator.name){
...
```{r}
estimate.class.moderator.effect.full <- function(moderator.name){
...
moderator.model.results <- estimate.class.moderator.effect.full(moderator.name)
...
model.moderator.estimates <- moderator.model.results %>%
  ...
  mutate(moderator.effect.un.z = moderator.effect * sd(classes[[moderator.name]]) + mean(classes[[moderator.name]])) %>%
  mutate(mu.condition.effect.un.z = mu.condition.effect - mean(classes[[moderator.name]])) %>%
  group_by(incentive) %>%
  sample_n(20)
...
ggplot(model.class.estimates, aes(x=admission.rate, y=.value, ymin=.lower, ymax=.upper, color=incentive)) +
  ...
  geom_abline(data=model.moderator.estimates,
              aes(intercept = mu.condition.effect.un.z, slope=moderator.effect.un.z, color=factor(incentive)),
              alpha=0.2)
```
