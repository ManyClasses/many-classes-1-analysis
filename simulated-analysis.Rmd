...
```{r}
ggplot(student.class.data %>% 
         group_by(class.id, admission.rate, incentive) %>% 
         summarize(m=mean(delta.z), se=sd(delta.z)/sqrt(n())),
  aes(x=admission.rate, y=m, ymin=m-se, ymax=m+se, color=incentive)) +
  geom_pointrange()
```
