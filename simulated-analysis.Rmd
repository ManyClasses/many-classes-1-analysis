...
model.1.result <- run.jags(
  model='no-moderators-jags-model.txt', 
  data=jags.data,
  n.chains = 3,
  sample = 1000,
  burnin = 1000,
  monitor = c('mu.condition','sigma.condition')
)
```

## Effect of Feedback Timing in Different Incentive Conditions

What's the overall mean for delta z-scores for the two conditions? Values above 0 indicate a benefit of immediate feedback. Values below 0 indicate a benefit of delayed feedback.

How much variance is there in the delta z-scores for the two conditions?

```{r}
model.1.result$summaries
```
